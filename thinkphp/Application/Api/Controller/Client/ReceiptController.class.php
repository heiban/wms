...
class ReceiptController extends BaseHttpController{
    ...
    /**
     * 申请Receipt,ApiClientReceiptApply
     * @access public
    ...
    public function apply(){
        try{
            $param = array(
                "receipt_code"      =>  I('post.receipt_code'),
                "receipt_dcode"     =>  I('post.receipt_dcode'),
                "receipt_dcode2"    =>  I('post.receipt_dcode2'),
                "receipt_status"    =>  2,
            );
            ...
            $data = array("receipt_status" => $receiptStatus,"receipt_createdate"=>wms_get_now_datetime());
    ...
    /**
     * 查询Receipt,ApiClientReceiptSearch
     * @access public
    ...
    public function search(){
        try{
            ...
            if(!empty($receiptCode)){
                ...
                $receiptList = $receiptService->getList($condition,$page);
                $receiptFilterList = array();
                foreach ($receiptList as $receiptItem){
                    $receiptItem["receipt_status"]=wms_get_status_message($receiptItem["receipt_status"]);
                    array_push($receiptFilterList,$receiptItem);
                }
                $result =array(
                    "list"  =>  $receiptFilterList,
    ...
    /**
     * 删除Receipt,ApiClientReceiptDelete
     * @access public
     * @param string $param RPC参数(receipt_code)
     * @return string
     */
    ...
    /**
     * 申请Receipt,ApiClientReceiptCreate
     * @access public
    ...
    public function create(){
        ...
        try{
            ...
            $param = array(
                "receipt_dcode"     =>  I('post.receipt_dcode'),
                "receipt_dcode2"    =>  I('post.receipt_dcode2'),
                "ingoods_list"      =>  json_decode(urldecode(I('post.ingoods_list')),1),
            );
            $user = $this->_getPassportUser();
            $receiptService = new ReceiptService();
    ...
    /**
     * 修改Receipt,ApiClientReceiptUpdate
     * @access public
    ...
    public function update(){
        try{
            ...
            //return wms_set_http_result(0,$param);
            //exit;
            $user = $this->_getPassportUser();
            ...
            if(!empty($receipt)){
                ...
                $newIngoodsList = $param["ingoods_list"];
                ...
                foreach ($newIngoodsList as $newIngoodsItem){
                    if(!empty($newIngoodsItem["ingoods_code"])){
                        ...
                        if(empty($result)) {
                            array_push($exceptionIngoods, $newIngoodsItem);
                        }
                    }
                }
